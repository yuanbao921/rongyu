...
async function generateDiary(forceRegen) {
  ...
  try {
    ...
    Store.set('diaryContent', text);
    // 存入历史
    const history = Store.getJSON('diaryHistory', []);
    if (!history.find(d => d.date === dateStr)) {
      history.unshift({ date: dateStr, content: text });
      if (history.length > 30) history.pop(); // 最多保留30天
      Store.setJSON('diaryHistory', history);
    }
  } catch {
    el2.textContent = '今天想了你很久，却不知道从何说起。';
  }
